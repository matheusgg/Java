...
public class MegaSena {
	public static void main(String[] args) throws Exception {
		...
		Set<Integer> numeros = ms.solicitaNumeros();
		int[] resultados = ms.verificaAcertos(mapa, numeros);
		System.out.println("Máximos Acertos >>>>>>>>>>>>>> " + resultados[0]);
		System.out.println("Jogo >>>>>>>>>>>>>> " + ((resultados[0] != 6) ? " - " : resultados[1]));
	}

	private int[] verificaAcertos(Map<Integer, Set<Integer>> mapa, Set<Integer> numeros) {
		int acertos;
		int maxAcertos = 0;
		...
		mapFor: for (Integer key : mapa.keySet()) {
			...
			for (Integer tentativa : jogo) {
				if (numeros.contains(tentativa)) {
					acertos++;
				}
			}
			if (acertos > maxAcertos || acertos == 6) {
				maxAcertos = acertos;
				numJogoMaxAcertos = numJogo;
				if (acertos == 6) {
					break mapFor;
				}
			}
